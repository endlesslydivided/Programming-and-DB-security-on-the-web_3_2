...
go
declare @g1 geometry; 
select @g1 = ogr_geometry from MAP where ogr_fid = 101;
declare @g2 geometry; 
...
as
return
	select CityName [Название города],min((geometry::STGeomFromText('Point('+ CAST(@latitude as nvarchar(max)) +' ' + CAST(@longitude as nvarchar(max)) + ')', 0)).STDistance(Point)) [Растояние] from cityCoord inner join City on cityCoord.id = City.City_Addr group by CityName;
