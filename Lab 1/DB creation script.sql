...
DeparturePoint varchar(100) NOT NULL constraint FK_CITYD_ROUTE foreign key (DeparturePoint) references City(CityName) ON DELETE CASCADE,
...
ServiceType varchar(50) NOT NULL constraint FK_SERVICE_SERVICETYPE foreign key (ServiceType) references ServiceType(ServiceName) ON DELETE CASCADE,
RouteName varchar(100) NOT NULL constraint FK_SERVICE_ROUTE foreign key (RouteName) references Route(RouteName) ON DELETE CASCADE,
...
CustomerName varchar(300) NOT NULL constraint FK_ORDER_CUSTOMER foreign key (CustomerName) references Customer(CustomerName) ON DELETE CASCADE,
ServiceId int NOT NULL constraint FK_ORDER_SERVICE foreign key (ServiceId) references Service(Id) ON DELETE CASCADE,
