...
spool "migration transdb_&filename..log"

-- Password file execution
@passworddefinition.sql

PROMPT Dropping Role ROLE_migration_transdb ...
DROP ROLE ROLE_migration_transdb ;
PROMPT Creating Role ROLE_migration_transdb ...
CREATE ROLE ROLE_migration_transdb ;

-- PROMPT Drop TRANS user
-- drop user TRANS cascade;
   
PROMPT Create user TRANS
CREATE USER TRANS IDENTIFIED BY &&TRANS_password PASSWORD EXPIRE ACCOUNT LOCK /* DEFAULT TABLESPACE USERS  TEMPORARY TABLESPACE TEMP */;
GRANT CREATE SESSION, RESOURCE, CREATE VIEW, CREATE MATERIALIZED VIEW, CREATE SYNONYM, UNLIMITED TABLESPACE TO TRANS;
...
GRANT ALL ON utils TO ROLE_migration_transdb;
grant ALL on utils to TRANS with grant option;

set define on
prompt connecting to TRANS
alter session set current_schema=TRANS;
set define off
-- DROP TABLE Order_ CASCADE CONSTRAINTS;


PROMPT Creating Table Order_ ...
CREATE TABLE Order_ (
  Id NUMBER(10,0) GENERATED BY DEFAULT ON NULL AS IDENTITY START WITH 1 INCREMENT BY 1 MINVALUE 1 NOMAXVALUE ,
  CustomerName VARCHAR2(300 CHAR) NOT NULL,
  ServiceId NUMBER(10,0) NOT NULL,
  UnitsAmount NUMBER(6,1) NOT NULL,
  OrderDate DATE NOT NULL,
  OrderExec DATE NOT NULL
);


COMMENT ON TABLE Order_ IS 'ORIGINAL NAME:Order'
;

ALTER TABLE Order_ MODIFY (UnitsAmount DEFAULT '0');
PROMPT Creating Primary Key Constraint PK_Order on table Order_ ... 
ALTER TABLE Order_
...
GRANT ALL ON Order_ TO ROLE_migration_transdb;
...
CREATE TABLE City (
  Id NUMBER(10,0) GENERATED BY DEFAULT ON NULL AS IDENTITY START WITH 1 INCREMENT BY 1 MINVALUE 1 NOMAXVALUE ,
  CityName VARCHAR2(100 CHAR) NOT NULL,
  City_Addr NUMBER(10,0)
);


PROMPT Creating Primary Key Constraint PK_CITY on table City ... 
ALTER TABLE City
...
ENABLE
;
PROMPT Creating Unique Constraint UQ__City__886159E57BAF37BF on table City
ALTER TABLE City
ADD CONSTRAINT UQ__City__886159E57BAF37BF UNIQUE (
  CityName
)
ENABLE
;

GRANT ALL ON City TO ROLE_migration_transdb;
-- DROP TABLE TR_AUDIT CASCADE CONSTRAINTS;


PROMPT Creating Table TR_AUDIT ...
CREATE TABLE TR_AUDIT (
  ID NUMBER(10,0) GENERATED BY DEFAULT ON NULL AS IDENTITY START WITH 1 INCREMENT BY 1 MINVALUE 1 NOMAXVALUE ,
  STMT VARCHAR2(20 CHAR),
  TRNAME VARCHAR2(50 CHAR),
  CC VARCHAR2(800 CHAR)
);


PROMPT Creating Check Constraint CK__TR_AUDIT__STMT__5070F446 on table TR_AUDIT... 
ALTER TABLE TR_AUDIT
ADD CONSTRAINT CK__TR_AUDIT__STMT__5070F446 CHECK 
(
   ( STMT = 'UPD'
                  OR STMT = 'DEL'
                  OR STMT = 'INS' )
...
GRANT ALL ON TR_AUDIT TO ROLE_migration_transdb;
...
CREATE TABLE Service (
  Id NUMBER(10,0) GENERATED BY DEFAULT ON NULL AS IDENTITY START WITH 1 INCREMENT BY 1 MINVALUE 1 NOMAXVALUE ,
  ServiceType VARCHAR2(50 CHAR) NOT NULL,
  RouteName VARCHAR2(100 CHAR) NOT NULL,
  CostPerUnit NUMBER(19,4) NOT NULL
);


ALTER TABLE Service MODIFY (CostPerUnit DEFAULT '0');
PROMPT Creating Primary Key Constraint PK_SERVICE on table Service ... 
ALTER TABLE Service
...
GRANT ALL ON Service TO ROLE_migration_transdb;
-- DROP TABLE ServiceType CASCADE CONSTRAINTS;


PROMPT Creating Table ServiceType ...
CREATE TABLE ServiceType (
  Id NUMBER(10,0) GENERATED BY DEFAULT ON NULL AS IDENTITY START WITH 1 INCREMENT BY 1 MINVALUE 1 NOMAXVALUE ,
  ServiceName VARCHAR2(50 CHAR) NOT NULL,
  UnitType VARCHAR2(15 CHAR) NOT NULL
);


PROMPT Creating Unique Constraint UQ__ServiceT__A42B5F9932C0131F on table ServiceType
ALTER TABLE ServiceType
ADD CONSTRAINT UQ__ServiceT__A42B5F9932C0131F UNIQUE (
  ServiceName
)
ENABLE
;
PROMPT Creating Primary Key Constraint PK_SERVICE_TYPE on table ServiceType ... 
ALTER TABLE ServiceType
ADD CONSTRAINT PK_SERVICE_TYPE PRIMARY KEY
(
  Id
)
ENABLE
;
PROMPT Creating Check Constraint CK__ServiceTy__UnitT__25869641 on table ServiceType... 
ALTER TABLE ServiceType
ADD CONSTRAINT CK__ServiceTy__UnitT__25869641 CHECK 
(
   ( UnitType = 'Человек'
                  OR UnitType = 'Килограмм'
                  OR UnitType = 'Литр' )
) 
ENABLE
;

GRANT ALL ON ServiceType TO ROLE_migration_transdb;
-- DROP TABLE cityCoord CASCADE CONSTRAINTS;


PROMPT Creating Table cityCoord ...
...
PROMPT Creating Primary Key Constraint TOWN_ID_PK on table cityCoord ... 
ALTER TABLE cityCoord
ADD CONSTRAINT TOWN_ID_PK PRIMARY KEY
(
  Id
)
ENABLE
...
GRANT ALL ON cityCoord TO ROLE_migration_transdb;
...
GRANT ALL ON geometry_columns TO ROLE_migration_transdb;
...
PROMPT Creating Primary Key Constraint PK_CUSTOMER on table Customer ... 
...
ENABLE
;
PROMPT Creating Unique Constraint UQ__Customer__7A22C6EA5C7B4FF6 on table Customer
ALTER TABLE Customer
ADD CONSTRAINT UQ__Customer__7A22C6EA5C7B4FF6 UNIQUE (
  CustomerName
)
ENABLE
;

GRANT ALL ON Customer TO ROLE_migration_transdb;
...
CREATE TABLE Geo (
  ...
  LocationType NVARCHAR2(9)
);


COMMENT ON COLUMN Geo.Level_ IS 'ORIGINAL NAME:Level'
;


GRANT ALL ON Geo TO ROLE_migration_transdb;
...
CREATE TABLE map (
  ...
  pop2005 NUMBER(10,0),
  region NUMBER(3,0),
  subregion NUMBER(3,0),
  lon NUMBER(8,3),
  lat NUMBER(7,3)
);


PROMPT Creating Primary Key Constraint PK_map on table map ... 
ALTER TABLE map
ADD CONSTRAINT PK_map PRIMARY KEY
(
  ogr_fid
)
ENABLE
...
GRANT ALL ON map TO ROLE_migration_transdb;
...
PROMPT Creating Primary Key Constraint PK__spatial___36B11BD5F17A7FF1 on table spatial_ref_sys ... 
ALTER TABLE spatial_ref_sys
ADD CONSTRAINT PK__spatial___36B11BD5F17A7FF1 PRIMARY KEY
(
  srid
)
ENABLE
...
GRANT ALL ON spatial_ref_sys TO ROLE_migration_transdb;
...
CREATE TABLE Route (
  ...
  DeparturePoint VARCHAR2(100 CHAR) NOT NULL,
  ArrivalPoint VARCHAR2(100 CHAR) NOT NULL
);


PROMPT Creating Unique Constraint UQ__Route__D3FE5EDB0F8F9A07 on table Route
ALTER TABLE Route
ADD CONSTRAINT UQ__Route__D3FE5EDB0F8F9A07 UNIQUE (
  RouteName
)
ENABLE
;
PROMPT Creating Primary Key Constraint PK_ROUTE on table Route ... 
ALTER TABLE Route
...
GRANT ALL ON Route TO ROLE_migration_transdb;
PROMPT Creating Index INDEX_ORDERCNDESC_2 on Order_ ...
CREATE INDEX INDEX_ORDERCNDESC_2 ON Order_
(
  CustomerName
) 
;
PROMPT Creating Index INDEX_ORDERSIDDESC_5 on Order_ ...
CREATE INDEX INDEX_ORDERSIDDESC_5 ON Order_
(
  ServiceId
) 
;
PROMPT Creating Index INDEX_ORDERUADESC_6 on Order_ ...
CREATE INDEX INDEX_ORDERUADESC_6 ON Order_
(
  UnitsAmount
) 
;
PROMPT Creating Index INDEX_ORDEROEDESC_4 on Order_ ...
CREATE INDEX INDEX_ORDEROEDESC_4 ON Order_
(
  OrderDate
) 
;
PROMPT Creating Index INDEX_ORDERODDESC_3 on Order_ ...
CREATE INDEX INDEX_ORDERODDESC_3 ON Order_
(
  OrderExec
) 
;
PROMPT Creating Index INDEX_SERVICESTDESC_9 on Service ...
CREATE INDEX INDEX_SERVICESTDESC_9 ON Service
(
  ServiceType
) 
;
PROMPT Creating Index INDEX_SERVICERNDESC_8 on Service ...
CREATE INDEX INDEX_SERVICERNDESC_8 ON Service
(
  RouteName
) 
;
PROMPT Creating Index INDEX_SERVICECPUDESC_7 on Service ...
CREATE INDEX INDEX_SERVICECPUDESC_7 ON Service
(
  CostPerUnit
) 
;
PROMPT Creating Index ogr_dbo_map_ogr_geometry_sidx on map ...
CREATE INDEX ogr_dbo_map_ogr_geometry_sidx ON map
(
  ogr_geometry
) 
;
...
alter session set current_schema=TRANS;
set define off
CREATE OR REPLACE FORCE VIEW OrdersWithServices 
AS 
   SELECT Order_.Id ,
          CustomerName ,
          ServiceType ,
          ...
          UnitsAmount ,
          CostPerUnit ,
          (UTILS.CONVERT_TO_VARCHAR2(UnitsAmount * CostPerUnit,30) || ' $') TotalAmount  ,
          OrderDate ,
          OrderExec 
     FROM Order_ 
            JOIN Service    ON Order_.ServiceId = Service.Id;


GRANT ALL ON OrdersWithServices TO ROLE_migration_transdb;
CREATE OR REPLACE FORCE VIEW NumberRouteOrders 
AS 
   SELECT B.RouteName Название_маршрута  ,
          COUNT(A.RouteName)  Количество_поездок  
     FROM ( ( SELECT RouteName 
              FROM Order_ 
                     JOIN Service    ON ServiceId = Service.Id ) A
            RIGHT JOIN ( SELECT RouteName 
                         FROM Route  ) B   ON A.RouteName = B.RouteName
             ) 
     GROUP BY B.RouteName;


GRANT ALL ON NumberRouteOrders TO ROLE_migration_transdb;
CREATE OR REPLACE FORCE VIEW CityRoutesDeparture 
AS 
   SELECT CityName ,
          COUNT(Route.RouteName)  Количество_путей_из_этого_города  
     FROM City 
            LEFT JOIN Route    ON Route.DeparturePoint = CityName
     GROUP BY CityName;


GRANT ALL ON CityRoutesDeparture TO ROLE_migration_transdb;
CREATE OR REPLACE FUNCTION getDistanse
(
  v_idCity IN NUMBER,
  v_idCityT IN NUMBER
)
RETURN FLOAT
AS
   v_g1 UDT.GEOMETRY%TYPE;
   v_g2 UDT.GEOMETRY%TYPE;
...
END;
/
GRANT ALL ON getDistanse TO ROLE_migration_transdb;
/*Global Temporary Tables:1 *//* Translation Extracted DDL For Required Objects*/
CREATE GLOBAL TEMPORARY TABLE tt_TGetOrdersByDate
AS (
   SELECT * 
     FROM Order_ 
...
/
CREATE OR REPLACE PACKAGE TGetOrdersByDate_pkg
AS
   TYPE tt_TGetOrdersByDate_type IS TABLE OF tt_TGetOrdersByDate%ROWTYPE;
END;
/


CREATE OR REPLACE FUNCTION TGetOrdersByDate
(
  v_start IN DATE,
  v_end IN DATE
)
RETURN TGetOrdersByDate_pkg.tt_TGetOrdersByDate_type PIPELINED
AS
   v_temp SYS_REFCURSOR;
   v_temp_1 TT_TGETORDERSBYDATE%ROWTYPE;

BEGIN
   DELETE tt_TGetOrdersByDate;
   
   INSERT INTO tt_TGetOrdersByDate
      SELECT * 
        FROM Order_ 
       WHERE  OrderDate BETWEEN v_start AND v_end;

      OPEN v_temp FOR
      SELECT * 
        FROM tt_TGetOrdersByDate;
...
END;
/
GRANT ALL ON TGetOrdersByDate TO ROLE_migration_transdb;
CREATE OR REPLACE FUNCTION TGetOrdersQByCompany
(
...
BEGIN
   SELECT COUNT(*)  
     ...
     INTO v_temp
     FROM Order_ 
    WHERE  CustomerName = v_companyName;
   RETURN v_temp;

EXCEPTION WHEN OTHERS THEN utils.handleerror(SQLCODE,SQLERRM);
END;
/
GRANT ALL ON TGetOrdersQByCompany TO ROLE_migration_transdb;
/*Global Temporary Tables:1 *//* Translation Extracted DDL For Required Objects*/
CREATE GLOBAL TEMPORARY TABLE tt_TGetRelevantRoutesByComp
AS (
   SELECT C.RouteName Название_пути  ,
   NVL(0 , '0') Количество_поездок  
     FROM Service A
     JOIN Order_ B   ON A.Id = B.ServiceId
     LEFT JOIN Route C   ON A.RouteName = C.RouteName
    
);
/
CREATE OR REPLACE PACKAGE TGetRelevantRoutesByCompany_pkg
AS
   TYPE tt_TGetRelevantRoutesByComp_type IS TABLE OF tt_TGetRelevantRoutesByComp%ROWTYPE;
END;
/


CREATE OR REPLACE FUNCTION TGetRelevantRoutesByCompany
(
  v_companyName IN VARCHAR2
)
RETURN TGetRelevantRoutesByCompany_pkg.tt_TGetRelevantRoutesByComp_type PIPELINED
AS
   v_temp SYS_REFCURSOR;
   v_temp_1 TT_TGETRELEVANTROUTESBYCOMP%ROWTYPE;

BEGIN
   DELETE tt_TGetRelevantRoutesByComp;
   
   INSERT INTO tt_TGetRelevantRoutesByComp
      SELECT C.RouteName Название_пути  ,
             NVL(COUNT(B.Id) , '0') Количество_поездок  
        FROM Service A
               JOIN Order_ B   ON A.Id = B.ServiceId
               LEFT JOIN Route C   ON A.RouteName = C.RouteName
       WHERE  B.CustomerName = v_companyName
        GROUP BY C.RouteName;

      OPEN v_temp FOR
      SELECT * 
        FROM tt_TGetRelevantRoutesByComp;
...
END;
/
GRANT ALL ON TGetRelevantRoutesByCompany TO ROLE_migration_transdb;
CREATE OR REPLACE PROCEDURE addChildNode
(
  v_LocationParent IN NVARCHAR2,
  v_LocationChild IN NVARCHAR2,
  v_LocationType IN NVARCHAR2
)
AS
   v_Level UDT.HIERARCHYID%TYPE;
   v_RLevel UDT.HIERARCHYID%TYPE;

BEGIN

   SELECT ( SELECT LEVEL_ 
            FROM Geo 
             WHERE  Location = v_LocationParent ) 
   ...
   SELECT MAX(LEVEL_)  
     ...
     INTO v_Level
     FROM Geo 
    WHERE  LEVEL_.GetAncestor(1) = v_RLevel;Geo VALUES ( @RLevel . GetDescendant ( @Level , NULL ) , @LocationChild , @LocationType ) ;  --SQLDEV: NOT RECOGNIZED

EXCEPTION WHEN OTHERS THEN utils.handleerror(SQLCODE,SQLERRM);
END;
/
GRANT ALL ON addChildNode TO ROLE_migration_transdb;
CREATE OR REPLACE PROCEDURE AI_ORDERS
AS
   v_CustomerName VARCHAR2(300);
   v_ServiceId NUMBER(10,0);
   v_UnitsAmount NUMBER(10,0);
   v_OrderDate DATE;
   v_OrderExec DATE;
         ...
         SELECT c1 text_  
           ...
           FROM ( SELECT c1 
                  FROM ( SELECT CustomerName 
                         FROM Customer  ) T1
                    ORDER BY ABS(/*TODO:SQLDEV*/ CHECKSUM(NEWID()) /*END:SQLDEV*/)
         ...
         SELECT c1 text_  
           ...
           FROM ( SELECT c1 
                  FROM ( SELECT Id 
                    ...
                    ORDER BY ABS(/*TODO:SQLDEV*/ CHECKSUM(NEWID()) /*END:SQLDEV*/)
                    FETCH FIRST 1 ROWS ONLY ) T2;
         SELECT utils.dateadd('SECOND', FLOOR(utils.rand() * 60), utils.dateadd('MINUTE', FLOOR(utils.rand() * 60), utils.dateadd('HOUR', FLOOR(utils.rand() * 24), utils.dateadd('DAY', FLOOR(utils.rand() * 500), '20180101')))) 

           INTO v_OrderDate
           FROM DUAL ;
         SELECT utils.dateadd('MINUTE', FLOOR(utils.rand() * 60), utils.dateadd('HOUR', FLOOR(utils.rand() * 24), utils.dateadd('DAY', FLOOR(utils.rand() * 500), v_OrderDate))) 

           INTO v_OrderExec
           FROM DUAL ;
         SELECT FLOOR(utils.rand() * (100 - 30) + 30) 

           INTO v_UnitsAmount
           FROM DUAL ;
         INSERT INTO Order_
           ( CustomerName, ServiceId, UnitsAmount, OrderDate, OrderExec )
           VALUES ( v_CustomerName, v_ServiceId, v_UnitsAmount, v_OrderDate, v_OrderExec );
         v_number := v_number + 1 ;
...
END;
/
GRANT ALL ON AI_ORDERS TO ROLE_migration_transdb;
...
BEGIN
   ...
   OPEN  v_cursor FOR
      SELECT LEVEL_.ToString() Связи  ,
             * 
        FROM Geo 
       WHERE  LEVEL_.GetLevel() = v_level ;
      DBMS_SQL.RETURN_RESULT(v_cursor);

EXCEPTION WHEN OTHERS THEN utils.handleerror(SQLCODE,SQLERRM);
END;
/
GRANT ALL ON getRoot TO ROLE_migration_transdb;
CREATE OR REPLACE PROCEDURE TCity
AS
   v_cursor SYS_REFCURSOR;
...
END;
/
GRANT ALL ON TCity TO ROLE_migration_transdb;
CREATE OR REPLACE PROCEDURE TCustomer
AS
   v_cursor SYS_REFCURSOR;
...
END;
/
GRANT ALL ON TCustomer TO ROLE_migration_transdb;
CREATE OR REPLACE PROCEDURE TOrder
AS
   v_cursor SYS_REFCURSOR;
...
END;
/
GRANT ALL ON TOrder TO ROLE_migration_transdb;
CREATE OR REPLACE PROCEDURE TRoute
AS
   v_cursor SYS_REFCURSOR;
...
END;
/
GRANT ALL ON TRoute TO ROLE_migration_transdb;
CREATE OR REPLACE PROCEDURE TServiceType
AS
   v_cursor SYS_REFCURSOR;
...
END;
/
GRANT ALL ON TServiceType TO ROLE_migration_transdb;
CREATE OR REPLACE PROCEDURE TServices
AS
   v_cursor SYS_REFCURSOR;
...
END;
/
GRANT ALL ON TServices TO ROLE_migration_transdb;
set define on
prompt connecting to TRANS
alter session set current_schema=TRANS;
set define off
PROMPT Creating Foreign Key Constraint CADDR_ID_FK on table cityCoord...
ALTER TABLE City
ADD CONSTRAINT CADDR_ID_FK FOREIGN KEY
(
  City_Addr
)
REFERENCES cityCoord
(
  Id
)
ENABLE
...
PROMPT Creating Foreign Key Constraint FK_CITYD_ROUTE on table City...
ALTER TABLE Route
ADD CONSTRAINT FK_CITYD_ROUTE FOREIGN KEY
(
  DeparturePoint
)
REFERENCES City
...
PROMPT Creating Foreign Key Constraint FK_CITYA_ROUTE on table City...
ALTER TABLE Route
ADD CONSTRAINT FK_CITYA_ROUTE FOREIGN KEY
(
  ArrivalPoint
)
REFERENCES City
...
PROMPT Creating Foreign Key Constraint FK_SERVICE_ROUTE on table Route...
ALTER TABLE Service
ADD CONSTRAINT FK_SERVICE_ROUTE FOREIGN KEY
(
  RouteName
)
REFERENCES Route
...
PROMPT Creating Foreign Key Constraint FK_SERVICE_SERVICETYPE on table ServiceType...
ALTER TABLE Service
ADD CONSTRAINT FK_SERVICE_SERVICETYPE FOREIGN KEY
(
  ServiceType
)
...
PROMPT Creating Foreign Key Constraint FK_ORDER_CUSTOMER on table Customer...
ALTER TABLE Order_
...
PROMPT Creating Foreign Key Constraint FK_ORDER_SERVICE on table Service...
ALTER TABLE Order_
ADD CONSTRAINT FK_ORDER_SERVICE FOREIGN KEY
(
  ServiceId
)
REFERENCES Service
...
ALTER VIEW OrdersWithServices COMPILE;

ALTER VIEW NumberRouteOrders COMPILE;

ALTER VIEW CityRoutesDeparture COMPILE;
